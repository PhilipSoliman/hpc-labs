...
int main(int argc, char *argv[])
{
  ...
  double timeSeq, timePar, /* timing variables */
      startTime, timeArray[MAX_MATRIX_SIZE_P2 - MIN_MATRIX_SIZE_P2 + 1][4];
  ...
  for (int n = MIN_MATRIX_SIZE_P2; n <= MAX_MATRIX_SIZE_P2; n++)
  {
    ...
    if (tid == 0)
    {
      ...
      timeArray[3][n - MIN_MATRIX_SIZE_P2] = N;
    }
  ...
  if (tid == 0)
  {
    ...
    saveArray(&timeArray, MAX_MATRIX_SIZE_P2 - MIN_MATRIX_SIZE_P2 + 1, 4, fullPath);
    printf("Saving done!\n");
  }
